...
const { Box, Button, Entry, EventBox, Icon, Label, Scrollable, Overlay } =
  Widget;
...
const conversions = [
  ...
  //   unit1Default: 5,
  //   formula1to2:
  //     '{{x}} / (parseFloat(Utils.exec(\'gsettings get org.gnome.desktop.interface font-name\').split(" ").pop().split("\'"))*4/3)',
  //   formula2to1:
  //     '{{x}} * (parseFloat(Utils.exec(\'gsettings get org.gnome.desktop.interface font-name\').split(" ").pop().split("\'"))*4/3)',
  //   forcePrecision: true,
  // },
...
export default () => {
  ...
  const BidirectionalConversion = ({
    ...
    precision = VALUE_DEFAULT_PRECISION,
  }) => {
    ...
    const convert = (value, formula) => {
      ...
      const result = forcePrecision
        ? evalResult.toFixed(precision)
        : truncateToPrecision(evalResult, precision);
      // print(result)
      return result;
    };
    ...
    const unit2Box = ValueBox({
      unit: unit2,
      initValue: truncateToPrecision(
        eval(formula1to2.replace("\{{x}}", unit1Default)),
        precision,
      ),
      updateCallback: (self) => {
        if (updateLock) return;
    ...
    return Box({
      ...
      children: [unit1Box, MaterialIcon("swap_horiz", "large"), unit2Box],
    });
  };
